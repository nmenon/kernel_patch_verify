...
export DTSCHEMA_REV=v2022.03.2
...
download_and_install_armgcc()
{
	...
	tar -C /usr/local/cross-gcc-linux-9/ --strip-components=1 -xvf "$F64"
	rm -f "$F64"
	...
	tar -C /usr/local/cross-gcc-linux-9/ --strip-components=1 -xvf "$F32"
	rm -f "$F32"
}
